...
module.exports = {
    ...
    // devOnly: Boolean,
    // testOnly: Boolean,
    allowedUsers: devs + ["961731087632068699", "142906427000881152"],
    options: [
        {
            name: "name",
...
*/
    ...
    callback: async (client, interaction) => {
        ...
        try {
            if (nameInput.includes("shio")) {
                if(misc_keywords.feral_cat.some(word => nameInput.includes(word))) { active_character = character.shiori_kitten }
                else if(misc_keywords.feral_dog.some(word => nameInput.includes(word))) { active_character = character.shiori_puppy }
                else if(misc_keywords.feral_bunny.some(word => nameInput.includes(word))) { active_character = character.shiori_bunny }
                else if(misc_keywords.feral_dragon.some(word => nameInput.includes(word))) { active_character = character.shiori_dragon }
                else { active_character = character.shiori }
            }
            else if (nameInput.includes("min")) { 
                if(misc_keywords.smol.some(word => nameInput.includes(word))) { active_character = character.min_smol }
                else { active_character = character.min }
            }
            else if (nameInput.includes("kiyo")) { active_character = character.kiyoshi }
            else if (nameInput.includes("fam")) { active_character = character.familiar }
            else if (nameInput.includes("yum")) { active_character = character.yumi }
            else if (nameInput.includes("ko")) { active_character = character.koko }
            else {
                interaction.reply({
